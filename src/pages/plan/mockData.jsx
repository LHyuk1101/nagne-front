export const mockPlanData = {
  subject: "서울의 자연과 문화, 아모레 성수에서 시작하는 특별한 여정",
  dayPlans: [
    {
      day: 1,

      places: [
        {
          order: 1,
          placeId: 6,
          title: "Amore Seongsu (아모레 성수)",
          moveTime: 0,
          placeSummary:
            "아모레 성수는 아모레퍼시픽의 체험 매장으로, 독특한 분위기에서 다양한 화장품을 체험할 수 있습니다.",
          reasoning:
            "여행의 시작을 아모레 성수에서 화장품 체험으로 하여, 한국의 뷰티 문화를 느끼고 기분 좋게 시작할 수 있습니다.",
          contentTypeId: "80",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
        {
          order: 2,
          placeId: 3,
          title: "Achasan Ecological Park (아차산생태공원)",
          moveTime: 5,
          placeSummary:
            "아차산생태공원은 자연과 생태를 체험할 수 있는 공간으로, 다양한 시설이 마련되어 있습니다.",
          reasoning:
            "아모레 성수에서 가까운 이곳에서 자연을 느끼며 편안한 산책을 즐길 수 있어, 여행의 첫날을 여유롭게 마무리할 수 있습니다.",
          contentTypeId: "76",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
      ],
    },
    {
      day: 2,
      places: [
        {
          order: 1,
          placeId: 4,
          title: "Achasan Mountain (아차산)",
          moveTime: 10,
          placeSummary:
            "아차산은 서울에서 쉽게 접근할 수 있는 하이킹 명소로, 정상에서 멋진 경치를 감상할 수 있습니다.",
          reasoning:
            "하이킹을 통해 서울의 아름다운 경치를 감상하며, 자연 속에서 활력을 얻을 수 있는 좋은 기회입니다.",
          contentTypeId: "82",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
        {
          order: 2,
          placeId: 5,
          title: "Achasanseong Fortress (아차산성)",
          moveTime: 5,
          placeSummary:
            "아차산성은 역사적인 요새로, 한강과 서울의 스카이라인을 감상할 수 있는 멋진 장소입니다.",
          reasoning:
            "하이킹 후 역사적인 장소를 방문하여, 서울의 과거와 현재를 동시에 느낄 수 있는 경험을 제공합니다.",
          contentTypeId: "76",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
      ],
    },
    {
      day: 3,
      places: [
        {
          order: 1,
          placeId: 1,
          title: "63 Square (63스퀘어)",
          moveTime: 15,
          placeSummary:
            "63스퀘어는 서울의 상징적인 랜드마크로, 멋진 전경과 현대 미술을 감상할 수 있습니다.",
          reasoning:
            "서울의 경제 성장과 현대 문화를 느낄 수 있는 장소로, 다양한 전시와 함께 멋진 경치를 즐길 수 있습니다.",
          contentTypeId: "76",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
        {
          order: 2,
          placeId: 2,
          title: "A Moment in Time - Live Caricature & Gallery (시간을 담다)",
          moveTime: 10,
          placeSummary:
            "전통과 현대가 어우러진 캐리커처 갤러리로, 특별한 기념품을 만들 수 있습니다.",
          reasoning:
            "63스퀘어 방문 후, 한국의 전통을 느끼며 재미있는 기념품을 만들 수 있는 기회를 제공합니다.",
          contentTypeId: "76",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
      ],
    },
    {
      day: 4,
      places: [
        {
          order: 1,
          placeId: 6,
          title: "Amore Seongsu (아모레 성수)",
          moveTime: 0,
          placeSummary:
            "아모레 성수에서 다양한 화장품을 체험하고 구매할 수 있습니다.",
          reasoning:
            "여행 중간에 다시 아모레 성수를 방문하여, 이전에 놓친 제품을 체험하고 구매할 수 있는 기회를 제공합니다.",
          contentTypeId: "80",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
        {
          order: 2,
          placeId: 3,
          title: "Achasan Ecological Park (아차산생태공원)",
          moveTime: 5,
          placeSummary: "자연을 느끼며 편안한 산책을 즐길 수 있는 공간입니다.",
          reasoning:
            "아모레 성수에서의 쇼핑 후, 자연 속에서 휴식을 취하며 여행의 피로를 풀 수 있는 좋은 장소입니다.",
          contentTypeId: "76",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
      ],
    },
    {
      day: 5,
      places: [
        {
          order: 1,
          placeId: 4,
          title: "Achasan Mountain (아차산)",
          moveTime: 10,
          placeSummary:
            "아차산에서 하이킹을 통해 서울의 아름다운 경치를 감상할 수 있습니다.",
          reasoning:
            "자연 속에서의 하이킹은 여행의 피로를 풀고 새로운 에너지를 얻을 수 있는 좋은 방법입니다.",
          contentTypeId: "82",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
        {
          order: 2,
          placeId: 5,
          title: "Achasanseong Fortress (아차산성)",
          moveTime: 5,
          placeSummary:
            "아차산성에서 한강과 서울의 스카이라인을 감상할 수 있습니다.",
          reasoning:
            "하이킹 후 역사적인 장소를 방문하여, 서울의 과거와 현재를 동시에 느낄 수 있는 경험을 제공합니다.",
          contentTypeId: "76",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
      ],
    },
    {
      day: 6,
      places: [
        {
          order: 1,
          placeId: 1,
          title: "63 Square (63스퀘어)",
          moveTime: 15,
          placeSummary:
            "서울의 상징적인 랜드마크로, 멋진 전경과 현대 미술을 감상할 수 있습니다.",
          reasoning:
            "여행의 마지막 날, 서울의 경제 성장과 현대 문화를 느낄 수 있는 장소로, 다양한 전시와 함께 멋진 경치를 즐길 수 있습니다.",
          contentTypeId: "76",
          placeImg:
            "http://tong.visitkorea.or.kr/cms/resource/86/2526386_image2_1.jpg",
        },
        {
          order: 2,
          placeId: 2,
          title: "A Moment in Time - Live Caricature & Gallery (시간을 담다)",
          moveTime: 10,
          placeSummary:
            "전통과 현대가 어우러진 캐리커처 갤러리로, 특별한 기념품을 만들 수 있습니다.",
          reasoning:
            "63스퀘어 방문 후, 한국의 전통을 느끼며 재미있는 기념품을 만들 수 있는 기회를 제공합니다.",
          contentTypeId: "76",
          placeImg: "https://example.com/images/achasan_park.jpg",
        },
      ],
    },
  ],
};
